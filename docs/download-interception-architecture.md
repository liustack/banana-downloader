...
**为什么不在 main world 中用 JS patch 抑制原生下载？**

最初尝试过 patch `HTMLAnchorElement.prototype.click` 来拦截 `blob:` 下载，但不可靠：
- Gemini 可能使用 `dispatchEvent(new MouseEvent('click'))` 而非 `.click()`，绕过 prototype patch
- 页面可能创建脱离 DOM 的 `<a>` 元素，事件不会冒泡到 document
- `CustomEvent.detail` 无法跨 isolated/main world 传递（变为 `null`）

最终方案：在 background 层使用 `chrome.downloads.onCreated` 监听并取消非本插件发起的 `blob:` 下载。这是浏览器级别的 API，无论页面如何触发下载都能可靠拦截。
